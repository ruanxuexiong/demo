...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	static final String LOG_TAG = "Widget_PullToRefresh";
	 ...
	 * 
	 * @param a - TypedArray of Widget_PullToRefresh Attributes
	 */
	...
	private void init(Context context, AttributeSet attrs) {
		...
		// Styleables from XML
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.Widget_PullToRefresh);

		if (a.hasValue(R.styleable.Widget_PullToRefresh_anl_ptrMode)) {
			mMode = Mode.mapIntToValue(a.getInteger(R.styleable.Widget_PullToRefresh_anl_ptrMode, 0));
		}

		if (a.hasValue(R.styleable.Widget_PullToRefresh_anl_ptrAnimationStyle)) {
			mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(
					R.styleable.Widget_PullToRefresh_anl_ptrAnimationStyle, 0));
		}
		 ...
		 * Styleables from XML
		 */
		if (a.hasValue(R.styleable.Widget_PullToRefresh_anl_ptrRefreshableViewBackground)) {
			Drawable background = a.getDrawable(R.styleable.Widget_PullToRefresh_anl_ptrRefreshableViewBackground);
			if (null != background) {
				mRefreshableView.setBackgroundDrawable(background);
			}
		} else if (a.hasValue(R.styleable.Widget_PullToRefresh_anl_ptrAdapterViewBackground)) {
			Utils.warnDeprecation("ptrAdapterViewBackground", "ptrRefreshableViewBackground");
			Drawable background = a.getDrawable(R.styleable.Widget_PullToRefresh_anl_ptrAdapterViewBackground);
			if (null != background) {
				mRefreshableView.setBackgroundDrawable(background);
			}
		}

		if (a.hasValue(R.styleable.Widget_PullToRefresh_anl_ptrOverScroll)) {
			mOverScrollEnabled = a.getBoolean(R.styleable.Widget_PullToRefresh_anl_ptrOverScroll, true);
		}

		if (a.hasValue(R.styleable.Widget_PullToRefresh_anl_ptrScrollingWhileRefreshingEnabled)) {
			mScrollingWhileRefreshingEnabled = a.getBoolean(
					R.styleable.Widget_PullToRefresh_anl_ptrScrollingWhileRefreshingEnabled, false);
		}
	...
	public static enum AnimationStyle {
		/**
		 * This is the default for Android-Widget_PullToRefresh. Allows you to use any
